...
namespace OP_Engine.Characters
{
    public class Animator
    {
        ...
        public virtual void UpdateGear(Character character)
        {
            foreach (Item item in character.Inventory.Items)
            {
                if (item.Texture != null)
                {
                    item.Region = new Rectangle(character.Region.X, character.Region.Y, character.Region.Width, character.Region.Height);
                    item.Image = new Rectangle(character.Image.X, character.Image.Y, character.Image.Width, character.Image.Height);
                }
            }
        }
